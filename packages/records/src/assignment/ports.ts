...
export class PrismaAssignmentPorts implements AssignmentPorts {
   ...
   * `apps/web/src/lib/auth/actor.ts`. Ordered by id so the fallback is the
   * same person on every call rather than whoever the planner returned first;
   * a rotating fallback would make "why did this land here" unanswerable.
   *
