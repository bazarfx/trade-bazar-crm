 ...
 * to move — but this file is now itself a package the worker compiles, so the
 * move waits for the webhook slice that will be its second caller.
 */
...
import { serialiseMany, type RecordRow } from './serialise.js';
...
interface DelegateShape {
   ...
   * (password hash, role, invite flow — spec §5.5, `apps/web/src/lib/config/users.ts`) and
